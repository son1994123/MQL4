...
//---
   ...
   for(int i=0;i<OrdersTotal();i++)
     {
     string comment = "Buy lien tuc thu: "+ (i+1);
      if(Get_Value_Number(Bid - Get_Order_Open_Price(i)) == 0.0050)
        {
         if(Count_Magic_Number_144() == 1 && Count_Magic_Number_145() == i)
           {
            BUY(0.03,145,comment);
           }
        }
      if(Get_Value_Number(Bid - Get_Order_Open_Price(i+1)) == -0.0045)
        {
