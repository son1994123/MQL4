...
extern int MagicNumber=144;
extern double Lots =0.1;
extern double StopLoss=NULL;
extern double TakeProfit=NULL;
extern int TrailingStop=NULL;
...
int start()
{
  ...
  {
     int result=0;
     if((Ask<iBands(NULL,0,20,2,0,PRICE_CLOSE,MODE_LOWER,0))) // Here is your open buy rule
     {
     ...
     if((Ask<iBands(NULL,0,20,2,0,PRICE_CLOSE,MODE_UPPER,0))) // Here is your open Sell rule
     {
  ...
  for(int cnt=0;cnt<OrdersTotal();cnt++)
     {
      ...
      if(OrderType()<=OP_SELL &&   
         ...
         )  
        {
         if(OrderType()==OP_BUY)  
           {
              if((Ask<iMA(NULL,0,5,0,MODE_SMA,PRICE_CLOSE,0))) //here is your close buy rule
              {
           ...
           {
                if((Bid<iMA(NULL,0,5,0,MODE_SMA,PRICE_CLOSE,0))) // here is your close sell rule
                {
                   OrderClose(OrderTicket(),OrderLots(),OrderClosePrice(),Slippage,Red);
                }
            if(TrailingStop>0)  
